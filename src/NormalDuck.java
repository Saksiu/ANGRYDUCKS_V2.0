...
public class NormalDuck extends Entity
{
    public NormalDuck(JFrame frame,Player player,int initialPosY,boolean onLeft)
    {
    ...
    protected void move(){
        if(onLeft)
            entity.setBounds(entity.getX()+4,initialPosY,40,40);
        else
            entity.setBounds(entity.getX()-4,initialPosY,40,40);
    }
    @Override
    ...
    protected ImageIcon setImage(){
        if(onLeft)
            return new ImageIcon("img/normalduck-right.png");
        return new ImageIcon("img/normalduck-left.png");
    }
    @Override
